...
const EditEvent = () => {
  ...
  const appliedTemplateProps = useMemo(() => {
    ...
    const hasImage = entries.some(([key]) => key === "image" || key === "");
    const filteredEntries = hasImage
      ? entries.filter(([key]) => key !== "image_url")
      : entries;
    ...
    const finalEntries = mappedEntries.filter(([key]) => key !== "image" && key !== "");

    return Object.fromEntries(finalEntries);
  }, [templateFields, previewKey]);
            ...
            <div className="bg-white rounded-[10px] overflow-hidden h-64 md:h-[550px] relative group">
              {isLoadingTemplate ? (
                ...
                <div className="w-full h-full overflow-auto">
                  <LoadedTemplate key={`${previewKey}-${JSON.stringify(appliedTemplateProps)}`} {...appliedTemplateProps} />
                </div>
              )}
              <button
